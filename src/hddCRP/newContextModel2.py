...
def create_pop_model(grp : str, n_trials : int = 50, fold_turns : bool = False, last : bool = False) -> populationOneBackRewardContextModel:
    ...
    for ii,sub in enumerate(subs):
        seqs,rs = dl.load_raw_with_reward_phase2(sub) # NOTE: this function should be checked: figures out the reward info from the csv files
    ...
    model = populationOneBackRewardContextModel(Y, R, base_measure=base_measure)
    return model
...
def create_indiv_model(sub : str, n_trials : int = 50, fold_turns : bool = False, last : bool = False) -> populationOneBackRewardContextModel:
    ...
    seqs,rs = dl.load_raw_with_reward_phase2(sub) # NOTE: this function should be checked: figures out the reward info from the csv files
        # will raise an exception if the raw data files aren't in the correct spot
    seqs = np.concatenate(seqs);
    rs = np.concatenate(rs);
